...
double ln2(double x)
{
    return (double)(log(x) / log((double)2));
}
...
vector <z_type> DFT(vector <z_type> inputVectorX)
{
    ...
    for (size_t k = 0; k < N; k++)
    {
        ...
        {
            tempSum += inputVectorX[j] * exp(-(2 * M_PI * z_type(0.0, 1.0) * (double)k * (double)j) / (double)N);
        }
...
vector <z_type> IDFT(vector <z_type> inputVectorY)
{
    ...
    for (size_t k = 0; k < N; k++)
    {
        ...
        for (size_t j = 0; j < N; j++)
        {
            tempSum += inputVectorY[j] * exp((2 * M_PI * z_type(0.0, 1.0) * (double)k * (double)j) / (double)N);
        }
...
{
    int n = (int)ln2((double)inputVectorX.size());
    ...
    for (int k = 1; k < n + 1; k++)
    {
        ...
        {
            omega = exp(-(2 * j * M_PI * z_type(0.0, 1.0)) / (double)pow2(k));
            for (int l = 0; l < lLimit; l++)
            {
                outputVectorY[j * pow2(n - k) + l] = (inputVectorX[j * pow2(n - k + 1) + l] +
                    omega * inputVectorX[j * pow2(n - k + 1) + l + pow2(n - k)]) / sqrt(2);

                outputVectorY[pow2(n - 1) + j * pow2(n - k) + l] = (inputVectorX[j * pow2(n - k + 1) + l] -
                    omega * inputVectorX[j * pow2(n - k + 1) + l + pow2(n - k)]) / sqrt(2);
            }
        }
...
{
    int n = (int)ln2((double)inputVectorY.size());
    ...
    for (int k = 1; k < n + 1; k++)
    {
        ...
        for (int j = 0; j < jLimit; j++)
        {
            ...
            for (int l = 0; l < lLimit; l++)
            {
                outputVectorX[j * pow2(n - k) + l] = (inputVectorY[j * pow2(n - k + 1) + l] +
                    omega * inputVectorY[j * pow2(n - k + 1) + l + pow2(n - k)]) / sqrt(2);

                outputVectorX[pow2(n - 1) + j * pow2(n - k) + l] = (inputVectorY[j * pow2(n - k + 1) + l] -
                    omega * inputVectorY[j * pow2(n - k + 1) + l + pow2(n - k)]) / sqrt(2);
            }
        }
...
vector <z_type> FFT_wp(vector <z_type> inputVectorX)
{
    ...
    for (int k = 1; k < n + 1; k++)
    {
        ...
        for (int j = 0; j < jLimit; j++)
        {
            for (int l = 0; l < lLimit; l++)
            {
                omega = exp(-(2 * l * M_PI * z_type(0.0, 1.0)) / (double)pow2(k));

                outputVectorY[j * pow2(k) + l] = (inputVectorX[j * pow2(k) + l] +
                    omega * inputVectorX[j * pow2(k) + l + pow2(k - 1)]) / sqrt(2);

                outputVectorY[pow2(k - 1) + j * pow2(k) + l] = (inputVectorX[j * pow2(k) + l] -
                    omega * inputVectorX[j * pow2(k) + l + pow2(k - 1)]) / sqrt(2);
            }
        }
    ...
    for (int k = 1; k < n + 1; k++)
    {
        ...
        for (int j = 0; j < jLimit; j++)
        {
            for (int l = 0; l < lLimit; l++)
            {
                ...
                outputVectorX[j * pow2(k) + l] = (inputVectorY[j * pow2(k) + l] +
                    omega * inputVectorY[j * pow2(k) + l + pow2(k - 1)]) / sqrt(2);

                outputVectorX[pow2(k - 1) + j * pow2(k) + l] = (inputVectorY[j * pow2(k) + l] -
                    omega * inputVectorY[j * pow2(k) + l + pow2(k - 1)]) / sqrt(2);
            }
        }
...
vector<z_type> FFTConvolution(vector<z_type> X, vector<z_type> Y)
{
    ...
    int N = pow2((int)ceil(ln2((double)max(L, M))));
    ...
    else
    {
        ...
        if (string(argv[1]) == "DFT_FFT_TIME_TEST")
        {
            ...
            else
            {
                ...
                size_t N = X.size();
                size_t n_max = (size_t)ln2((double)N);
                size_t N_current = N;
                ...
                for (size_t n = 0; n < n_max; n++)
                {
                    ...
                    cout << "DFT/FFT time ratio: " << (int)(seconds_DFT / seconds_FFT) << endl;
                    cout << "---------------------------------------" << endl;
        ...
        if (string(argv[1]) == "CONV_FCONV_TIME_TEST")
        {
            ...
            else
            {
                ...
                if (max(L, M) < 10)
                {
                    cout << "It won't be useful :) Create input arrays with ~1000 elements at least" << endl;
                    system("pause");
                    exit(1);
                }
                ...
                for (M_current = M; M_current > M / 10; M_current -= M / 10)
                {
                    ...
                    cout << "CONV//FCONV time ratio: " << seconds_CONV / seconds_FCONV << endl;
                    cout << "---------------------------------------" << endl;
